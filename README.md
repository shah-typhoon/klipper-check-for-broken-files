...
После редактирования файла `printer.cfg` не забудьте сохранить файл и перегрузить `Klipper`.
...
Если возникла ошибка, посмотрите, в каком месте она отображается. Если за пределами сообщений `**Running Command {check_broken}...:**` и `**Command {check_broken} finished**`, значит это ошибка системы или макроса `CHECK_BROKEN`. А если между приведенными сообщениями, то это результат работы скрипта.
